...
from tsph_with_pst import VelocityGradient, DivGrad, MomentumEquationSecondOrder
...
def get_particle_array_sisph(constants=None, **props):
    sisph_props = [
        ...
        'uhat0', 'vhat0', 'what0', 'pabs', 'den', 'vmax', 'ki0', 'ki',
        'dii2', 'dii1', 'dii0', 'dijpj0', 'dijpj2', 'dijpj1', 'rhoc', 'cs'
    ]
...
class UpdateProperties(Equation):
    def initialize(self, d_idx, d_x, d_x0, d_y, d_y0, d_z, d_z0, d_p, d_u, d_v, d_w, d_gradv, d_gradp):
...
class VelocityGradientOrg(VelocityGradient):
    def initialize(self, d_idx, d_gradv, d_rho, d_rhoc):
        i = declare('int')
        for i in range(9):
...
class MomentumEquationSecondOrderOrg(MomentumEquationSecondOrder):
    def initialize(self, d_idx, d_au, d_av, d_aw, d_rho, d_rhoc):
        d_au[d_idx] = 0.0
        d_av[d_idx] = 0.0
        d_aw[d_idx] = 0.0
...
class DivGradOrg(DivGrad):
    def initialize(self, d_idx, d_au, d_av, d_aw, d_rhoc, d_rho):
        d_au[d_idx] = 0.0
        d_av[d_idx] = 0.0
        d_aw[d_idx] = 0.0
        d_rhoc[d_idx] = d_rho[d_idx]
...
class PressureCoeffMatrixIterative(Equation):
    ...
    def loop(self, d_idx, s_idx, s_m, d_rho, s_rho, d_diag, d_odiag, s_pk, XIJ,
        ...
        if R2IJ > 1e-14:
            fac = 4.0 * s_m[s_idx] * rhoij2_1 * xdotdwij / (R2IJ)
...
class ComputeDIJPJ(Equation):
    ...
    def loop(self, d_idx, d_dijpj0, d_dijpj1, d_dijpj2,
        ...
        fac = s_m[s_idx]/s_rho[s_idx]*s_p[s_idx]
...
class ComputeAII(Equation):
    ...
    def loop(self, d_idx, d_diag, d_dii0, d_dii1, d_dii2, d_m, d_rho,
             s_idx, s_m, s_rho, DWIJ, d_dijpj0, d_dijpj1, d_dijpj2, 
             s_dijpj0, s_dijpj1, s_dijpj2, s_dii0, s_dii1, s_dii2, d_odiag, s_pk, d_pk):
        facd = d_m[d_idx]/d_rho[d_idx]
        facs = s_m[s_idx]/s_rho[s_idx]
        dijdotdwij = (d_dii0[d_idx] - facd*DWIJ[0] * d_rho[d_idx]/s_rho[s_idx])*DWIJ[0] + \
                     (d_dii1[d_idx] - facd*DWIJ[1] * d_rho[d_idx]/s_rho[s_idx])*DWIJ[1] + \
                     (d_dii2[d_idx] - facd*DWIJ[2] * d_rho[d_idx]/s_rho[s_idx])*DWIJ[2]
        d_diag[d_idx] += facs*dijdotdwij/d_rho[d_idx]

        dijpj0 = d_dijpj0[d_idx]
        djjpj0 = s_dii0[s_idx]*s_pk[s_idx]
        djkpj0 = s_dijpj0[s_idx]
        val0 = (djkpj0  - djjpj0 - dijpj0) + facd*DWIJ[0]*d_pk[d_idx]

        dijpj1 = d_dijpj1[d_idx]
        djjpj1 = s_dii1[s_idx]*s_pk[s_idx]
        djkpj1 = s_dijpj1[s_idx]
        val1 = (djkpj1  - djjpj1 - dijpj1) + facd*DWIJ[1]*d_pk[d_idx]

        dijpj2 = d_dijpj2[d_idx]
        djjpj2 = s_dii2[s_idx]*s_pk[s_idx]
        djkpj2 = s_dijpj2[s_idx]
        val2 = (djkpj2  - djjpj2 - dijpj2) + facd*DWIJ[2]*d_pk[d_idx]

        d_odiag[d_idx] += (val0*DWIJ[0] + val1*DWIJ[1] + val2*DWIJ[2]) * facs / s_rho[s_idx]
...
class TISPHScheme(Scheme):
    ...
    def add_user_options(self, group):
        ...
        group.add_argument(
            ...
            help="method for pressure eval 'isph' or 'iisph'"
        )

    def consume_user_options(self, options):
        _vars = ['tolerance', 'omega', 'alpha', 'gtvf', 'symmetric',
                 'internal_flow', 'method']
    ...
    def _get_viscous_eqns(self):
        ...
        stg.append(Group(equations=eq, update_nnps=True))

        g1 = []
        for name in all:
            g1.append(GradientCorrectionPreStep(dest=name, sources=all, dim=self.dim))
        stg.append(Group(equations=g1, update_nnps=True))

        g2 = []
        for name in all:
            g2.append(GradientCorrection(dest=name, sources=all))
        for name in self.fluids:
            g2.append(VelocityGradientOrg(dest=name, sources=all))
        stg.append(Group(equations=g2, update_nnps=True))

        eq = []
        for fluid in self.fluids:
            for name in all:
                eq.append(GradientCorrection(dest=name, sources=all))
            if self.nu > 0.0:
                eq.append(
                    DivGradOrg(fluid, sources=self.fluids,
                                     nu=self.nu)
                )
            # if self.alpha > 0.0:
            #     eq.append(
    ...
    def _get_ppe(self):
        ...
        for name in all:
            eq2.append(GradientCorrection(dest=name, sources=all))
            eq2.append(
                VelocityDivergence(
                    dest=fluid,
                    sources=self.fluids
                ))
            if self.method == 'iisph':
                eq2.append(
                    ComputeDII(
                        dest=fluid,
                        sources=self.fluids
        ...
        stg.append(Group(equations=eq2, update_nnps=True))
        ...
        eq3 = []
        for name in all:
            eq3.append(GradientCorrection(dest=name, sources=all))
        for fluid in self.fluids:
            if not fluid == 'outlet':
                if self.method == 'iisph':
                    eq3.append(
                        ComputeAII(dest=fluid, sources=all)
                    )
                else:
                    eq3.append(
        ...
        eq3 = Group(equations=eq3, update_nnps=True)
    ...
    def get_equations(self):
        ...
        eq4 = []
        for name in all:
            eq4.append(GradientCorrection(dest=name, sources=all))
        for fluid in self.fluids:
            if self.symmetric:
                eq4.append(
                ...
                eq4.append(
                    MomentumEquationSecondOrderOrg(fluid, sources=all)
                )
            # if self.gtvf:
            #     eq4.append(
    ...
    def post_step(self, pa_arr, domain):
        if self.shifter is None:
            ...
            from tsph_with_pst import SaveInitialdistances, VelocityGradient, DensityGradient, IterativePSTNew
            ...
            equations.append(Group(equations=g3, iterate=True, min_iterations=1, max_iterations=1, update_nnps=True))

            g4 = []
            for name in self.fluids:
                g4.append(UpdateProperties(dest=name, sources=None))
            ...
            self.shifter = SPHEvaluator(
                ...
                kernel=self.solver.kernel, backend='cython', domain_manager=domain
            )
