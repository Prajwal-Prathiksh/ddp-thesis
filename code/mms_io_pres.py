...
def get_props(x, y, z, t, c0):
    ...
    v = (-x*(x - 1)*exp(-10*t)*sin(2*pi*y)) * np.ones_like(x)
    w = (0.0) * np.ones_like(x)
    p = (y*(y - 1)*exp(-10*t)*cos(2*pi*y)) * np.ones_like(x)
    rhoc = p/c0**2 + 1.0
...
class AddMomentumSourceTerm(Equation):
    ...
    def post_loop(self, d_au, d_av, d_aw, d_idx, d_x, d_y, d_z, d_u, d_v, d_w,
        ...
        v = -x*(x - 1)*exp(-10*t)*sin(2*pi*y)
        w = 0.0
        p = y*(y - 1)*exp(-10*t)*cos(2*pi*y)
        rhoc = p/c0**2 + rhoc0
        rho = d_rho[d_idx]

        d_au[d_idx] += v*(-2*pi*y*(y - 1)*exp(-10*t)*sin(2*pi*y) + y*exp(-10*t)*cos(2*pi*y) + (y - 1)*exp(-10*t)*cos(2*pi*y)) - 10*y*(y - 1)*exp(-10*t)*cos(2*pi*y) + 1.0*pi**2*y*(y - 1)*exp(-10*t)*cos(2*pi*y) + 1.0*pi*y*exp(-10*t)*sin(2*pi*y) + 1.0*pi*(y - 1)*exp(-10*t)*sin(2*pi*y) - 0.5*exp(-10*t)*cos(2*pi*y)
        d_av[d_idx] += u*(-x*exp(-10*t)*sin(2*pi*y) + (1 - x)*exp(-10*t)*sin(2*pi*y)) - 2*pi*v*x*(x - 1)*exp(-10*t)*cos(2*pi*y) - 1.0*pi**2*x*(x - 1)*exp(-10*t)*sin(2*pi*y) + 10*x*(x - 1)*exp(-10*t)*sin(2*pi*y) + 0.5*exp(-10*t)*sin(2*pi*y) + (-2*pi*y*(y - 1)*exp(-10*t)*sin(2*pi*y) + y*exp(-10*t)*cos(2*pi*y) + (y - 1)*exp(-10*t)*cos(2*pi*y))/rhoc
        d_aw[d_idx] += 0.0
...
class SetValuesonSolid(Equation):
    ...
    def initialize(
        ...
        d_gradv[idx9 + 2] = 0
        d_gradv[idx9 + 3] = -x*exp(-10*t)*sin(2*pi*y) + (1 - x)*exp(-10*t)*sin(2*pi*y)
        d_gradv[idx9 + 4] = -2*pi*x*(x - 1)*exp(-10*t)*cos(2*pi*y)
        d_gradv[idx9 + 5] = 0
        d_gradv[idx9 + 6] = 0
        d_gradv[idx9 + 7] = 0
        d_gradv[idx9 + 8] = 0
...
class AddContinuitySourceTerm(Equation):
    ...
    def post_loop(self, d_arho, d_idx, d_x, d_y, d_z, d_u, d_v, d_w, d_rho,
        ...
        d_arho[d_idx] += -2*pi*rhoc*x*(x - 1)*exp(-10*t)*cos(2*pi*y) + v*(-2*pi*y*(y - 1)*exp(-10*t)*sin(2*pi*y)/c0**2 + y*exp(-10*t)*cos(2*pi*y)/c0**2 + (y - 1)*exp(-10*t)*cos(2*pi*y)/c0**2) - 10*y*(y - 1)*exp(-10*t)*cos(2*pi*y)/c0**2
...
class AddPressureEvolutionSourceTerm(Equation):
    ...
    def post_loop(self, d_ap, d_idx, d_x, d_y, d_z, d_u, d_v, d_w, d_rho,
        ...
        d_ap[d_idx] += -800*pi*rhoc*x*(x - 1)*exp(-10*t)*cos(2*pi*y) + v*(-2*pi*y*(y - 1)*exp(-10*t)*sin(2*pi*y) + y*exp(-10*t)*cos(2*pi*y) + (y - 1)*exp(-10*t)*cos(2*pi*y)) - 10*y*(y - 1)*exp(-10*t)*cos(2*pi*y)
