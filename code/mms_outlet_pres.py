...
def get_props(x, y, z, t, c0):
    ...
    u = (y*(y - 1)*cos(2*pi*y) + 1) * np.ones_like(x)
    v = (0.0) * np.ones_like(x)
    w = (0.0) * np.ones_like(x)
    p = ((x - 1)**2*exp(-320000*(-t + x/40 - 0.0225)**2)*cos(2*pi*y)) * np.ones_like(x)
    rhoc = p/c0**2 + 1.0
...
class AddMomentumSourceTerm(Equation):
    ...
    def post_loop(self, d_au, d_av, d_aw, d_idx, d_x, d_y, d_z, d_u, d_v, d_w,
        ...
        u = y*(y - 1)*cos(2*pi*y) + 1
        v = 0.0
        w = 0.0
        p = (x - 1)**2*exp(-320000*(-t + x/40 - 0.0225)**2)*cos(2*pi*y)
        rhoc = p/c0**2 + rhoc0
        rho = d_rho[d_idx]

        d_au[d_idx] += v*(-2*pi*y*(y - 1)*sin(2*pi*y) + y*cos(2*pi*y) + (y - 1)*cos(2*pi*y)) + ((x - 1)**2*(16000*t - 400*x + 360.0)*exp(-320000*(-t + x/40 - 0.0225)**2)*cos(2*pi*y) + (2*x - 2)*exp(-320000*(-t + x/40 - 0.0225)**2)*cos(2*pi*y))/rhoc
        d_av[d_idx] += -2*pi*(x - 1)**2*exp(-320000*(-t + x/40 - 0.0225)**2)*sin(2*pi*y)/rhoc
        d_aw[d_idx] += 0.0
...
class SetValuesonSolid(Equation):
    ...
    def initialize(
        ...
        d_gradv[idx9] = 0
        d_gradv[idx9 + 1] = -2*pi*y*(y - 1)*sin(2*pi*y) + y*cos(2*pi*y) + (y - 1)*cos(2*pi*y)
        d_gradv[idx9 + 2] = 0
        d_gradv[idx9 + 3] = 0
        d_gradv[idx9 + 4] = 0
        d_gradv[idx9 + 5] = 0
...
class AddContinuitySourceTerm(Equation):
    ...
    def post_loop(self, d_arho, d_idx, d_x, d_y, d_z, d_u, d_v, d_w, d_rho,
        ...
        d_arho[d_idx] += u*((x - 1)**2*(16000*t - 400*x + 360.0)*exp(-320000*(-t + x/40 - 0.0225)**2)*cos(2*pi*y)/c0**2 + (2*x - 2)*exp(-320000*(-t + x/40 - 0.0225)**2)*cos(2*pi*y)/c0**2) - 2*pi*v*(x - 1)**2*exp(-320000*(-t + x/40 - 0.0225)**2)*sin(2*pi*y)/c0**2 + (x - 1)**2*(-640000*t + 16000*x - 14400.0)*exp(-320000*(-t + x/40 - 0.0225)**2)*cos(2*pi*y)/c0**2
...
class AddPressureEvolutionSourceTerm(Equation):
    ...
    def post_loop(self, d_ap, d_idx, d_x, d_y, d_z, d_u, d_v, d_w, d_rho,
        ...
        d_ap[d_idx] += u*((x - 1)**2*(16000*t - 400*x + 360.0)*exp(-320000*(-t + x/40 - 0.0225)**2)*cos(2*pi*y) + (2*x - 2)*exp(-320000*(-t + x/40 - 0.0225)**2)*cos(2*pi*y)) - 2*pi*v*(x - 1)**2*exp(-320000*(-t + x/40 - 0.0225)**2)*sin(2*pi*y) + (x - 1)**2*(-640000*t + 16000*x - 14400.0)*exp(-320000*(-t + x/40 - 0.0225)**2)*cos(2*pi*y)
