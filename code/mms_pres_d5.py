...
def get_props(x, y, z, t, c0):
    ...
    u = ((y - 1)*sin(2*pi*x)*cos(2*pi*y)) * np.ones_like(x)
    v = (-(y - 1)*sin(2*pi*y)*cos(2*pi*x)) * np.ones_like(x)
    w = (0.0) * np.ones_like(x)
    p = (atan2((y - 0.5)**2, (x - 0.5)**2)) * np.ones_like(x)
    rhoc = p/c0**2 + 1.0
...
class AddMomentumSourceTerm(Equation):
    ...
    def post_loop(self, d_au, d_av, d_aw, d_idx, d_x, d_y, d_z, d_u, d_v, d_w,
        ...
        u = (y - 1)*sin(2*pi*x)*cos(2*pi*y)
        v = -(y - 1)*sin(2*pi*y)*cos(2*pi*x)
        w = 0.0
        p = atan2((y - 0.5)**2, (x - 0.5)**2)
        rhoc = p/c0**2 + rhoc0
        rho = d_rho[d_idx]

        d_au[d_idx] += 2*pi*u*(y - 1)*cos(2*pi*x)*cos(2*pi*y) + v*(-2*pi*(y - 1)*sin(2*pi*x)*sin(2*pi*y) + sin(2*pi*x)*cos(2*pi*y)) - (2*x - 1.0)*(y - 0.5)**2/(rhoc*((x - 0.5)**4 + (y - 0.5)**4))
        d_av[d_idx] += -2*pi*u*(1 - y)*sin(2*pi*x)*sin(2*pi*y) + v*(2*pi*(1 - y)*cos(2*pi*x)*cos(2*pi*y) - sin(2*pi*y)*cos(2*pi*x)) + (x - 0.5)**2*(2*y - 1.0)/(rhoc*((x - 0.5)**4 + (y - 0.5)**4))
        d_aw[d_idx] += 0.0
...
class SetValuesonSolid(Equation):
    ...
    def initialize(
        ...
        d_gradv[idx9] = 2*pi*(y - 1)*cos(2*pi*x)*cos(2*pi*y)
        d_gradv[idx9 + 1] = -2*pi*(y - 1)*sin(2*pi*x)*sin(2*pi*y) + sin(2*pi*x)*cos(2*pi*y)
        d_gradv[idx9 + 2] = 0
        d_gradv[idx9 + 3] = -2*pi*(1 - y)*sin(2*pi*x)*sin(2*pi*y)
        d_gradv[idx9 + 4] = 2*pi*(1 - y)*cos(2*pi*x)*cos(2*pi*y) - sin(2*pi*y)*cos(2*pi*x)
        d_gradv[idx9 + 5] = 0
        d_gradv[idx9 + 6] = 0
        d_gradv[idx9 + 7] = 0
        d_gradv[idx9 + 8] = 0
...
class AddContinuitySourceTerm(Equation):
    ...
    def post_loop(self, d_arho, d_idx, d_x, d_y, d_z, d_u, d_v, d_w, d_rho,
        ...
        d_arho[d_idx] += rhoc*(2*pi*(1 - y)*cos(2*pi*x)*cos(2*pi*y) + 2*pi*(y - 1)*cos(2*pi*x)*cos(2*pi*y) - sin(2*pi*y)*cos(2*pi*x)) - u*(2*x - 1.0)*(y - 0.5)**2/(c0**2*((x - 0.5)**4 + (y - 0.5)**4)) + v*(x - 0.5)**2*(2*y - 1.0)/(c0**2*((x - 0.5)**4 + (y - 0.5)**4))
...
class AddPressureEvolutionSourceTerm(Equation):
    ...
    def post_loop(self, d_ap, d_idx, d_x, d_y, d_z, d_u, d_v, d_w, d_rho,
        ...
        d_ap[d_idx] += 400*rhoc*(2*pi*(1 - y)*cos(2*pi*x)*cos(2*pi*y) + 2*pi*(y - 1)*cos(2*pi*x)*cos(2*pi*y) - sin(2*pi*y)*cos(2*pi*x)) - u*(2*x - 1.0)*(y - 0.5)**2/((x - 0.5)**4 + (y - 0.5)**4) + v*(x - 0.5)**2*(2*y - 1.0)/((x - 0.5)**4 + (y - 0.5)**4)
