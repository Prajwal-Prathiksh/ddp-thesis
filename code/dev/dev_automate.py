...
# Local imports.
from automate_utils import styles, custom_compare_runs, SimulationPlotter
...
class SineVelProfilePlotters(SimulationPlotter):
    """
    ...
    def get_plotter_map(self):
        plotter_map = dict(
            ...
            l2_error=dict(
                ...
                xlabel=r'$log(k)$',
                ylabel=r'$log(L_2)$',
                title_main=r"($L_2$ error wrt exact solution)"
            ),
            l2_error_no_interp=dict(
                ...
                xlabel=r'$log(k)$',
                ylabel=r'$log(L_2)$',
                title_main=r"($L_2$ error wrt no interpolation solution)"
            )
        )
        return plotter_map
...
class SineVelProfile(PySPHProblem):
    ...
    def run(self):
        ...
        for dim, nx in zip(tmp['dim'], tmp['nx']):
            ...
            tmp_ob.plot_comparison(
                plot_type='l2_error', sims=fcases, labels=labels, title_suffix=title_suffix
            )
            tmp_ob.plot_comparison(
                plot_type='loglog', sims=fcases, labels=labels, title_suffix=title_suffix
            )
