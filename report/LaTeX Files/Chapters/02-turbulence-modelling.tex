...
Where $\RAProp{\vect{v}}$ is Reynolds-averaged velocity, and $L_m$ refers to the mixing length scales. The SPH formulation for the mean velocity gradients are given in \Eqref{eq:violeau-mean-velocity-gradient}.
...
Where $(k)$ is the turbulent kinetic energy, $(\epsilon)$ the dissipation rate, and $(C_i)$ being constants - $(C_1=1.8, C_2=0.6)$.
...
The stress tensor defined in \Eqref{eq:Gotoh2004-stress-tensor} is closed using Boussinesq’s Hypothesis as defined in \Eqref{eq:Gotoh2004-boussinesq}.
\begin{equation}
...
\begin{equation}
    \tensor{\tau} = \rho \bigg( 2\nu_t \tensor{S} - \frac{2 \nu_t}{3}\operatorname{tr}[\tensor{S}]\tensor{I} \bigg) - \frac{4}{3} \rho C_I \Delta^2 \tensor{I} \FrobeniusInnerProduct{S}{S} \quad , \quad C_I = \SciNot{6.6}{-3}
...
The authors used the  $2D$ and $3D$ TGV problems to validate the model. The authors considered Reynolds number up to $\approx \SciNot{1.2}{5}$ for the $2D$ case, and $\approx \SciNot{1.5}{3}$ for the $3D$ case.
...
The authors define a vorticity field in the system as given in \Eqref{eq:Liu2019-vorticity}.
\begin{equation}
...
A stream function is subsequently defined as given in \Eqref{eq:Liu2019-stream-func}.
\begin{equation}
