...
def get_props(x, y, z, t, c0):
    ...
    u = ((y - 0.5)*sin(2*pi*x)*cos(2*pi*y)) * np.ones_like(x)
    v = (-(x - 0.5)*sin(2*pi*x)*cos(2*pi*y)) * np.ones_like(x)
...
class AddMomentumSourceTerm(Equation):
    ...
    def post_loop(self, d_au, d_av, d_aw, d_idx, d_x, d_y, d_z, d_u, d_v, d_w,
        ...
        u = (y - 0.5)*sin(2*pi*x)*cos(2*pi*y)
        v = -(x - 0.5)*sin(2*pi*x)*cos(2*pi*y)
        w = 0.0
        p = cos(4*pi*x) + cos(4*pi*y)
        rhoc = p/c0**2 + rhoc0
        rho = d_rho[d_idx]

        d_au[d_idx] += 2*pi*u*(y - 0.5)*cos(2*pi*x)*cos(2*pi*y) + v*(-2*pi*(y - 0.5)*sin(2*pi*x)*sin(2*pi*y) + sin(2*pi*x)*cos(2*pi*y)) - 4*pi*sin(4*pi*x)/rhoc
        d_av[d_idx] += u*(2*pi*(0.5 - x)*cos(2*pi*x)*cos(2*pi*y) - sin(2*pi*x)*cos(2*pi*y)) - 2*pi*v*(0.5 - x)*sin(2*pi*x)*sin(2*pi*y) - 4*pi*sin(4*pi*y)/rhoc
        d_aw[d_idx] += 0.0
...
class SetValuesonSolid(Equation):
    ...
    def initialize(
        ...
        d_gradv[idx9] = 2*pi*(y - 0.5)*cos(2*pi*x)*cos(2*pi*y)
        d_gradv[idx9 + 1] = -2*pi*(y - 0.5)*sin(2*pi*x)*sin(2*pi*y) + sin(2*pi*x)*cos(2*pi*y)
        d_gradv[idx9 + 2] = 0
        d_gradv[idx9 + 3] = 2*pi*(0.5 - x)*cos(2*pi*x)*cos(2*pi*y) - sin(2*pi*x)*cos(2*pi*y)
        d_gradv[idx9 + 4] = -2*pi*(0.5 - x)*sin(2*pi*x)*sin(2*pi*y)
        d_gradv[idx9 + 5] = 0
        d_gradv[idx9 + 6] = 0
        d_gradv[idx9 + 7] = 0
        d_gradv[idx9 + 8] = 0
...
class AddContinuitySourceTerm(Equation):
    ...
    def post_loop(self, d_arho, d_idx, d_x, d_y, d_z, d_u, d_v, d_w, d_rho,
        ...
        d_arho[d_idx] += rhoc*(-2*pi*(0.5 - x)*sin(2*pi*x)*sin(2*pi*y) + 2*pi*(y - 0.5)*cos(2*pi*x)*cos(2*pi*y)) - 4*pi*u*sin(4*pi*x)/c0**2 - 4*pi*v*sin(4*pi*y)/c0**2
...
class AddPressureEvolutionSourceTerm(Equation):
    ...
    def post_loop(self, d_ap, d_idx, d_x, d_y, d_z, d_u, d_v, d_w, d_rho,
        ...
        d_ap[d_idx] += 400*rhoc*(-2*pi*(0.5 - x)*sin(2*pi*x)*sin(2*pi*y) + 2*pi*(y - 0.5)*cos(2*pi*x)*cos(2*pi*y)) - 4*pi*u*sin(4*pi*x) - 4*pi*v*sin(4*pi*y)
