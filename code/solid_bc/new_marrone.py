...
def boundary_props():
    ...
    '''
    return ['proj', 'V', {'name':'coeff', 'stride':10}]
...
def solid_bc(bcs, fluids, rho0, p0):
    print(bcs)
    from rkpm import Volume, EvaluateReproducingCoefficients, PressureApproxRKPM, VelocityApproxRKPM
    g0, g1, g2, g3 = [], [], [], []
    g0.append(
        Volume(dest='fluid', sources=['fluid'])
    )
    g1.extend([
        EvaluateReproducingCoefficients(dest='ghost_mirror', sources=['fluid']),
    ])
    # g2.extend([
    ...
    for bc in bcs:
        if bc == 'u_no_slip':
            for name in bcs[bc]:
                g2.append(
                    VelocityApproxRKPM(dest='ghost_mirror', sources=['fluid'])
                )
                g3.append(
        ...
        if bc == 'p_solid':
            for name in bcs[bc]:
                g2.append(
                    PressureApproxRKPM(dest='ghost_mirror', sources=['fluid'])
                )
                g3.append(
