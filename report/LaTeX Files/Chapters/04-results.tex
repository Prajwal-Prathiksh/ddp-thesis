...
However, it was decided that since such an implicit pressure Poisson equation solver would be computationally expensive, as well complicated to general to other problems, the scheme was implemented with a different SPH discretisation of the $k-\epsilon$ transport equations,so as to avoid that step.
...
\end{itemize}
For all of the above cases, the analytical RHS of the $k$ and $\epsilon$ transport equations ae computed using \texttt{SymPy} \parencite{Meurer2017}. a symbolic package, in order to automate the process, and avoid any errors in the analytical computation.

The results for the $k$ and $\epsilon$ transport equations are shown in \figref{fig:k-spatial-ooc} and \figref{fig:eps-spatial-ooc} respectively. It can be observed here, that for the first test-case, the OOC for the spatial operators in the transport equation agree well, irrespective of the variant of discretisation.
...
This can be attributed, to the fact the in the expanded equations of \eqref{eq:Shao2006-k-transport-eq-exp-res-chap} and \eqref{eq:Shao2006-eps-transport-eq-exp-res-chap}, the terms involve either higher-order derivatives or products of derivatives, which are not well approximated by the SPH operators, and hence, lead to increased errors. It appears, that the advantage of dealing with single-term derivatives (e.g, $\nabla k$) in the expanded equations, compared to having multi-term derivatives (e.g, $\nabla \cdot (k^2\nabla k / \epsilon)$) in the simplified equations, is not enough to offset the increased numerical errors due to the higher-order derivatives.
...
A similar study was undertaken to study the effects of $f_{pst}$ on the $k-\epsilon$ SPH scheme. The results are shown in \figref{fig:keps-pst}, where it can again be observed that the scheme appears to offer zeroth-order convergence for all values of $f_{pst}$.
Looking into the velocity magnitude field, as shown in \figref{fig:keps-pst-vmag}, similar trends are observed, in the that, higher values of $f_{pst}$ or lack of one, leads to clustering of particles, and hence, the velocity field appears jagged.
No other noticeable differences are observed in the energy spectrum, as shown in \figref{fig:keps-pst-espec}, which seems to indicate the $f_{pst}$ does not seem to have a strong effect on the energy spectrum, at least for short time simulations.
