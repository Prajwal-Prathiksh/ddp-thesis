...
def get_props(x, y, z, t, c0):
    ...
    p = (x**2 + cos(4*pi*x)) * np.ones_like(x)
    rhoc = p/c0**2 + 1.0
...
class AddMomentumSourceTerm(Equation):
    ...
    def post_loop(self, d_au, d_av, d_aw, d_idx, d_x, d_y, d_z, d_u, d_v, d_w,
        ...
        d_au[d_idx] += 2*pi*u*(y - 1)*cos(2*pi*x)*cos(2*pi*y) + v*(-2*pi*(y - 1)*sin(2*pi*x)*sin(2*pi*y) + sin(2*pi*x)*cos(2*pi*y)) + (2*x - 4*pi*sin(4*pi*x))/rhoc
        d_av[d_idx] += -2*pi*u*(1 - y)*sin(2*pi*x)*sin(2*pi*y) + v*(2*pi*(1 - y)*cos(2*pi*x)*cos(2*pi*y) - sin(2*pi*y)*cos(2*pi*x))
        d_aw[d_idx] += 0.0
...
class AddContinuitySourceTerm(Equation):
    ...
    def post_loop(self, d_arho, d_idx, d_x, d_y, d_z, d_u, d_v, d_w, d_rho,
        ...
        d_arho[d_idx] += rhoc*(2*pi*(1 - y)*cos(2*pi*x)*cos(2*pi*y) + 2*pi*(y - 1)*cos(2*pi*x)*cos(2*pi*y) - sin(2*pi*y)*cos(2*pi*x)) + u*(2*x - 4*pi*sin(4*pi*x))/c0**2
...
class AddPressureEvolutionSourceTerm(Equation):
    ...
    def post_loop(self, d_ap, d_idx, d_x, d_y, d_z, d_u, d_v, d_w, d_rho,
        ...
        d_ap[d_idx] += 400*rhoc*(2*pi*(1 - y)*cos(2*pi*x)*cos(2*pi*y) + 2*pi*(y - 1)*cos(2*pi*x)*cos(2*pi*y) - sin(2*pi*y)*cos(2*pi*x)) + u*(2*x - 4*pi*sin(4*pi*x))
