...
def get_props(x, y, z, t, c0):
    ...
    u = ((1 - y)**2*exp(-10*t)*sin(2*pi*x)*cos(2*pi*y)) * np.ones_like(x)
    v = (-(1 - y)**2*exp(-10*t)*sin(2*pi*y)*cos(2*pi*x)) * np.ones_like(x)
...
class AddMomentumSourceTerm(Equation):
    ...
    def post_loop(self, d_au, d_av, d_aw, d_idx, d_x, d_y, d_z, d_u, d_v, d_w,
        ...
        u = (1 - y)**2*exp(-10*t)*sin(2*pi*x)*cos(2*pi*y)
        v = -(1 - y)**2*exp(-10*t)*sin(2*pi*y)*cos(2*pi*x)
        w = 0.0
        p = (cos(4*pi*x) + cos(4*pi*y))*exp(-10*t)
        rhoc = p/c0**2 + rhoc0
        rho = d_rho[d_idx]

        d_au[d_idx] += 2*pi*u*(1 - y)**2*exp(-10*t)*cos(2*pi*x)*cos(2*pi*y) + v*(-2*pi*(1 - y)**2*exp(-10*t)*sin(2*pi*x)*sin(2*pi*y) + (2*y - 2)*exp(-10*t)*sin(2*pi*x)*cos(2*pi*y)) - 10*(1 - y)**2*exp(-10*t)*sin(2*pi*x)*cos(2*pi*y) + 2.0*pi**2*(1 - y)**2*exp(-10*t)*sin(2*pi*x)*cos(2*pi*y) + 1.0*pi*(2*y - 2)*exp(-10*t)*sin(2*pi*x)*sin(2*pi*y) - 0.5*exp(-10*t)*sin(2*pi*x)*cos(2*pi*y) - 4*pi*exp(-10*t)*sin(4*pi*x)/rhoc
        d_av[d_idx] += 2*pi*u*(1 - y)**2*exp(-10*t)*sin(2*pi*x)*sin(2*pi*y) + v*(-2*pi*(1 - y)**2*exp(-10*t)*cos(2*pi*x)*cos(2*pi*y) + (2 - 2*y)*exp(-10*t)*sin(2*pi*y)*cos(2*pi*x)) - 2.0*pi**2*(1 - y)**2*exp(-10*t)*sin(2*pi*y)*cos(2*pi*x) + 10*(1 - y)**2*exp(-10*t)*sin(2*pi*y)*cos(2*pi*x) - 0.5*pi*(2 - 2*y)*exp(-10*t)*cos(2*pi*x)*cos(2*pi*y) + 0.5*pi*(2*y - 2)*exp(-10*t)*cos(2*pi*x)*cos(2*pi*y) + 0.5*exp(-10*t)*sin(2*pi*y)*cos(2*pi*x) - 4*pi*exp(-10*t)*sin(4*pi*y)/rhoc
        d_aw[d_idx] += 0.0
...
class SetValuesonSolid(Equation):
    ...
    def initialize(
        ...
        d_gradv[idx9] = 2*pi*(1 - y)**2*exp(-10*t)*cos(2*pi*x)*cos(2*pi*y)
        d_gradv[idx9 + 1] = -2*pi*(1 - y)**2*exp(-10*t)*sin(2*pi*x)*sin(2*pi*y) + (2*y - 2)*exp(-10*t)*sin(2*pi*x)*cos(2*pi*y)
        d_gradv[idx9 + 2] = 0
        d_gradv[idx9 + 3] = 2*pi*(1 - y)**2*exp(-10*t)*sin(2*pi*x)*sin(2*pi*y)
        d_gradv[idx9 + 4] = -2*pi*(1 - y)**2*exp(-10*t)*cos(2*pi*x)*cos(2*pi*y) + (2 - 2*y)*exp(-10*t)*sin(2*pi*y)*cos(2*pi*x)
        d_gradv[idx9 + 5] = 0
        d_gradv[idx9 + 6] = 0
        d_gradv[idx9 + 7] = 0
        d_gradv[idx9 + 8] = 0
...
class AddContinuitySourceTerm(Equation):
    ...
    def post_loop(self, d_arho, d_idx, d_x, d_y, d_z, d_u, d_v, d_w, d_rho,
        ...
        d_arho[d_idx] += rhoc*(2 - 2*y)*exp(-10*t)*sin(2*pi*y)*cos(2*pi*x) - 4*pi*u*exp(-10*t)*sin(4*pi*x)/c0**2 - 4*pi*v*exp(-10*t)*sin(4*pi*y)/c0**2 - 10*(cos(4*pi*x) + cos(4*pi*y))*exp(-10*t)/c0**2
...
class AddPressureEvolutionSourceTerm(Equation):
    ...
    def post_loop(self, d_ap, d_idx, d_x, d_y, d_z, d_u, d_v, d_w, d_rho,
        ...
        d_ap[d_idx] += 400*rhoc*(2 - 2*y)*exp(-10*t)*sin(2*pi*y)*cos(2*pi*x) - 4*pi*u*exp(-10*t)*sin(4*pi*x) - 4*pi*v*exp(-10*t)*sin(4*pi*y) - 10*(cos(4*pi*x) + cos(4*pi*y))*exp(-10*t)
