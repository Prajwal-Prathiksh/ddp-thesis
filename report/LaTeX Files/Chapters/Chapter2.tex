...
In this scheme, the authors consider a transport equation for pressure instead of density derived from the continuity equation, as given in \ref{eq:Ramachandran2019-p-transport}.
\begin{equation}
    \LagDerivative{P} = -c_s^2 \rho \nabla \cdot \vect{v} + \nu \nabla^2 P
    \label{eq:Ramachandran2019-p-transport}
\end{equation}

Considering the pressure evolution equation above, an equation of state is longer for the scheme.
By filtering \ref{eq:Ramachandran2019-p-transport} with the Lagrangian LES technique \parencite{DiMascio2017}, we arrive at the result given by \ref{eq:edac-filtered} (Refer Appendix \ref{AppendixA} for derivation).
\begin{equation}
