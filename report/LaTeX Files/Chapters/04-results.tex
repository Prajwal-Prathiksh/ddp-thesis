...
From the previous section, it is evident that $k-\epsilon$ SPH and SPH-$\epsilon$ scheme need to be improved, before they can be considered as potential schemes for turbulence modelling in SPH.
...
\end{equation}
where $t*=tU/L$, $\vect{x}^* = \vect{x}/L$, and $A=1.3U^2/L$. $U$ and $L$ are the characteristic velocity and length scales of the system, respectively.
...
For the sake for uniformity, and computational feasibility, the same problem, is run by the L-IPST-C and $\delta$-LES-SPH schemes, with a resolution of $N=[100^2, 200^2]$, and other scheme-specific parameters as mentioned in \tabref{tab:opt-sph-schemes}.

The evolution of the kinetic energy for various $Re$ is shown in \figref{fig:tgv-forced-ke}.
...
In fact, this causes the L-IPST-C scheme to blow up for all $Re$ when $N=100^2$ even after the forcing term has been switched off. 
However, when the resolution is increased to $N=200^2$, the L-IPST-C scheme is able to track the trend as observed by Colagrossi, albeit with a slight over-prediction. This manifests in the energy appearing to diverge towards the end of the $3s$ mark. Therefore, it is probably safe to conclude the simmulation cannot be run for longer time (the author was able to run the simulation for $t_f=20s$) withough using a significantly large resolution.
On the other hand, the $\delta$-LES-SPH scheme, continues to underpredict the energy of the system, even for the $N=200^2$ case. There might be credible reasons for this, considering that the author used the same scheme, but with a resolution that is $6x$ finer. This would imply that if we consider the fact the $\delta$-LES-SPH scheme will continue to reduce the error in under-predicting the energy of the system, with increasing resolution, it is possible that the exact results obtained by Colagrossi, can be reproduced with simply a much higher resolution.
...
This would imply the lack of a strong viscous dissipation mechanism. This is also evident in the velocity magnitude plots, shown in \figref{fig:tgv-forced-colag-vmag}, where we can see a noisy velocity field, implying the presence of particles with relatively high energy throughout the domain. Similarly, in the vorticity plots, shown in \figref{fig:tgv-forced-colag-omega}, we see that the vorticity field is extremely noisy, and does not appear to have any coherent vorticity structures.
Therefore, it is reasonable to conclude that the reason for the L-IPST-C scheme to predict a slope of $-5/3$, might have more to do with the fact that the particles are no longer asscoiated with eddies, and are instead are being advected arbitrarily, without a viscous term to curtail their motionn and organise them into Lagrangian coherent structures.

On the other hand, the slope obtained from the $\delta$-LES-SPH scheme is $-3.12$, which is close to slope expected in the viscous subrange, but appearing in the inertial subrange instead. This shows that the $\delta$-LES-SPH scheme has a very strong viscous dissipation mechanism, which is causing a steep redistribution of the energy from a much larger scale than expected. This does imply, that the velocity magnitude field would be much smoother, and the flow would be characterised by very larger eddies that contain the bulk of the energy, and a cascade of energy to lower scales through smaller and smaller structures. This is indeed what is seen in \figref{fig:tgv-forced-colag-vmag}.
Similar conclusion can be made from observing the vorticity field, as shown in \figref{fig:tgv-forced-colag-omega}. The vorticity field obtained from the $\delta$-LES-SPH scheme, is much smoother, and appears to still have distorted yet coherent vorticity structures.
This indicates that merely increasing the resolution of the L-IPST-C scheme, will not be sufficient to turn it into a suitable turbulence model. Instead, a mechanism to dissipate energy at smaller scales will have to be introduced, which will allow for the energy to cascade to smaller scales, and be dissipated at the viscous subrange.
...
This needs to be studied further, since the pressure field is an important quantity when it comes to the applications of turbulence modelling.
...
Finally, the evolution of the angular and linear momentum for the various schemes are shown in \figref{fig:tgv-forced-am-lm}. These quantities would have to be conserved here since the net force on the system is zero. The expected trend of improved bounds around the zero line with increased resolution is observed.
However, the L-IPST-C scheme is not as accurate as the $\delta$-LES-SPH scheme, when it comes to being bounded, or having similar zones of peaks and troughs.
